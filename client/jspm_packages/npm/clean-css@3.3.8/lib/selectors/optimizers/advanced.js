...
(function(process) {
  ...
  AdvancedOptimizer.prototype.mergeAdjacent = function(tokens) {
    ...
    for (var i = 0,
      ...
      if (lastToken[0] == 'selector' && stringifySelectors(token[1]) == stringifySelectors(lastToken[1])) {
        ...
        token[2] = [];
      } else if (lastToken[0] == 'selector' && stringifyBody(token[2]) == stringifyBody(lastToken[2]) && !this.isSpecial(stringifySelectors(token[1])) && !this.isSpecial(stringifySelectors(lastToken[1]))) {
  ...
  AdvancedOptimizer.prototype.reduceNonAdjacent = function(tokens) {
    ...
    for (var i = tokens.length - 1; i >= 0; i--) {
      ...
      var selectors = isComplexAndNotSpecial ? [selectorAsString].concat(token[1]) : [selectorAsString];
      for (var j = 0,
  ...
  AdvancedOptimizer.prototype.reduceComplexNonAdjacentCases = function(tokens, candidates) {
    ...
    allSelectors: for (var complexSelector in candidates) {
      ...
      var reducedBodies = [];
      var selectors = this.isSpecial(complexSelector) ? [complexSelector] : into[0].list;
